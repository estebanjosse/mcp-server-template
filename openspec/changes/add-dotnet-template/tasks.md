## 1. Template Assets
- [ ] 1.1 Create template configuration (template.json, content mapping, symbols) that mirrors the current repository structure while excluding build artifacts.
- [ ] 1.2 Implement template symbols for `--include-sample-tools`, `--include-tests`, `--http-host`, `--stdio-host`, and `--name`, including validation that fails when neither host option is supplied and supports selecting both.
- [ ] 1.3 Author automation or manual verification scripts that run `dotnet new mcp-server` with common flag combinations to confirm the scaffold restores, builds, and runs.
